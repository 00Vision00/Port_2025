...
const Free = ["当サイトは,Next.js/Typescript/Three.js/webGL使用してます"]; // 表示するテキストの配列
...
export default function Home() {
    return (
        <>
            ...
            <FreeText /> {/* 3つ目の MaskText コンポーネント */}
        </>
    );
}
...
export function MaskText() {
    ...
    const { ref, inView } = useInView({
        ...
        triggerOnce: false, // 1回のみ実行し、スクロールしてもリセットしない
    });
    ...
    const animation = {
        ...
        enter: (i: number) => ({
            ...
            transition: {
                ...
                delay: 0.075 * i, // 各テキストが順番に表示されるように遅延を設定
            },
        }),
    };
...
export function FreeText() {
    ...
    return (
        <motion.div ref={ref} className={styles.body}> {/* ref を設定し、この要素の表示状態を監視 */}
            {Free.map((text, index) => ( // phrase 配列をループし、テキストを表示
                <div key={index} className={styles.lineMask}> {/* 各テキストの親コンテナ */}
                    <motion.p
                        custom={index} // アニメーションの delay に index を利用するため
                        variants={animation} // アニメーション設定を適用
